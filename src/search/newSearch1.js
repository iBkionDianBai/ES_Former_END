...
function NewSearchPage1() {
    ...
    const columns = [
        {
            ...
            // 这里是渲染返回，就是表格里填什么
            render: (_, {keshi}) => {
                return (keshi || "").split(new RegExp(`(${input})`, "gi")).map((c, i) => c === input ?
                    <mark key={i}>{c}</mark> : c) || '-'
            },
            ...
            width: 200,
            align: 'center',
            sorter:(a,b)=>a.keshi.localeCompare(b.keshi)
        },
        {
            title: '刻石内容',
            dataIndex: 'keshineirong',
            key: 'keshineirong',
            render: (_, {keshineirong}) => {
                //  return keshineirong || '-'
                let s = (keshineirong || "").split(new RegExp(`(${input})`, "gi")).map((c, i) => c === input ?
                    <mark key={i}>{c}</mark> : c) || '-'
                return (
            ...
            width: 500,
            align: 'center',
            sorter:(a,b)=>a.keshineirong.localeCompare(b.keshineirong)
        },
        {
            title: '操作',
            dataIndex: 'operation',
    ...
    const onFinish = async () => {
        if (selected == 'all') {
            ...
            axios.post('http://172.18.7.119:8080/KeShiWenBen/fullTextSearch', data)
                .then(response => {
            ...
            axios.defaults.headers["Content-Type"] = "application/json";
            axios.post('http://172.18.7.119:8080/KeShiWenBen/select', data)
                .then(response => {
                    console.log(response, 'resres')
    ...
    const update = () => {
        ...
        axios.put('http://172.18.7.119:8080/KeShiWenBen', new_data)
            .then(response => {
    ...
    const del = (id) => {
        axios.defaults.headers["Content-Type"] = "application/json";
        console.log(id)
        axios.delete(`http://172.18.7.119:8080/KeShiWenBen/${id}`)
            .then(response => {
    ...
    const addSingle = () => {
        ...
        axios.post('http://172.18.7.119:8080/KeShiWenBen', [add_data])
            .then(response => {
                console.log(response)
    ...
    const downloadFile = () => { //fileurl文件地址（一般是接口返回）
        let fileurl = '/刻石文本信息模板.xlsx'
        let filename = '刻石文本信息模板.xlsx'
        ...
        // axios.get('http://172.18.7.119:8080/KeShiWenBen')
        //     .then(response => {
    ...
    const props = {
        name: 'multipartFile',
        multiple: false,
        action: 'http://172.18.7.119:8080/KeShiWenBen/upload',
        onChange(info) {
        ...
        // 查看上传的文件
        // beforeUpload(file) {
        //     axios.post('http://172.18.7.119:8080/KeShiWenBen/upload', file)
        //         .then(response => {
        //             // setShowElem(true)
        //             // messageApi.info('删除成功')
        //             // console.log(response)
    ...
    return (
        <div className="page">
            ...
            <Card size="lage" title="修改页面" extra={<Button
                ...
                >
                    <Form.Item
                        name='keshi'
                        label='刻石'
                    > <Input placeholder={update_data.keshi}
                             onChange={(value) => {
                                 new_data.keshi = value.target.value
                             }}
                    /> </Form.Item>
                    <Form.Item
                        name='keshineirong'
                        label='刻石内容'
                    > <Input placeholder={update_data.keshineirong}
                             onChange={(value) => {
                                 new_data.keshineirong = value.target.value
                             }}
                    /> </Form.Item>
                    <Form.Item>
            ...
            <Card size="lage" title="查看页面" extra={<Button
                ...
                <Descriptions title="刻石信息">
                    <Descriptions.Item label="刻石">{update_data.keshi}</Descriptions.Item>
                    <Descriptions.Item label="刻石内容">
                        <div style={{whiteSpace: 'pre-line'}}>{update_data.keshineirong}</div>
                    </Descriptions.Item>
                </Descriptions>
            </Card>
            ...
            <Card size="lage" title="单条信息新增页面" extra={<Button
                ...
                <Form form={form} name="control-hooks" style={{width: 400}}>
                    ...
                    > <Input onChange={(value) => {
                        add_data.keshi = value.target.value
                    }}
                    /> </Form.Item>
                    <Form.Item
                        name='keshineirong'
                        label='刻石内容'
                    > <Input onChange={(value) => {
                        add_data.keshineirong = value.target.value
                    }}
                    /> </Form.Item>
                    <Form.Item>
