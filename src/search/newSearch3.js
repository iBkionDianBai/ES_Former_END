...
function NewSearchPage3() {
    ...
    const columns = [
        {
            ...
            key: 'chutudi',
            render: (_, {chutudi}) => {
                return (chutudi || "").split(new RegExp(`(${input})`, "gi")).map((c, i) => c === input ?
                    <mark key={i}>{c}</mark> : c) || '-'
            },
            ellipsis: true,
            align: 'center',
            sorter:(a,b)=>a.chutudi.localeCompare(b.chutudi)
        },
        {
            title: '题目',
            dataIndex: 'timu',
            key: 'timu',
            render: (_, {timu}) => {
                let s = (timu || "").split(new RegExp(`(${input})`, "gi")).map((c, i) => c === input ?
                    <mark key={i}>{c}</mark> : c) || '-'
                return <div style={{whiteSpace: 'pre-line'}}>{s}</div>
            },
            ellipsis: true,
            align: 'center',
            sorter:(a,b)=>a.timu.localeCompare(b.timu)
        },
        {
            title: '文本',
            dataIndex: 'wenben',
            key: 'wenben',
            render: (_, {wenben}) => {
                return (wenben || "").split(new RegExp(`(${input})`, "gi")).map((c, i) => c === input ?
                    <mark key={i}>{c}</mark> : c) || '-'
            },
            ellipsis: true,
            align: 'center',
    ...
    const onFinish = async () => {
        if (selected == 'all') {
            ...
            axios.post('http://172.18.7.119:8080/JianDuWenBen/fullTextSearch', data)
                .then(response => {
            ...
            axios.defaults.headers["Content-Type"] = "application/json";
            axios.post('http://172.18.7.119:8080/JianDuWenBen/select', data)
                .then(response => {
                    console.log(response, 'resres')
    ...
    const update = () => {
        ...
        axios.put('http://172.18.7.119:8080/JianDuWenBen', new_data)
            .then(response => {
    ...
    const del = (id) => {
        axios.defaults.headers["Content-Type"] = "application/json";
        console.log(id)
        axios.delete(`http://172.18.7.119:8080/JianDuWenBen/${id}`)
            .then(response => {
    ...
    const addSingle = () => {
        ...
        axios.post('http://172.18.7.119:8080/JianDuWenBen', [add_data])
            .then(response => {
                console.log(response)
    ...
    const downloadFile = () => { //fileurl文件地址（一般是接口返回）
        let fileurl = '/简牍文本信息模板.xlsx'
        let filename = '简牍文本信息模板.xlsx'
        ...
        // axios.get('http://172.18.7.119:8080/JianDuWenBen')
        //     .then(response => {
    ...
    const props = {
        name: 'multipartFile',
        multiple: false,
        action: 'http://172.18.7.119:8080/JianDuWenBen/upload',
        onChange(info) {
        ...
        // 查看上传的文件
        // beforeUpload(file) {
        //     axios.post('http://172.18.7.119:8080/JianDuWenBen/upload', file)
        //         .then(response => {
        //             // setShowElem(true)
        //             // messageApi.info('删除成功')
        //             // console.log(response)
    ...
    return (
        <div className="page">
            ...
            <Card size="lage" title="修改页面" extra={<Button
                ...
                >
                    <Form.Item
                        name='chutudi'
                        label='出土地/单位'
                    > <Input placeholder={update_data.chutudi}
                             onChange={(value) => {
                                 new_data.chutudi = value.target.value
                             }}
                    /> </Form.Item>
                    <Form.Item
                        name='timu'
                        label='题目'
                    > <Input placeholder={update_data.timu}
                             onChange={(value) => {
                                 new_data.timu = value.target.value
                             }}
                    /> </Form.Item>
                    <Form.Item
                        name='wenben'
            ...
            <Card size="lage" title="查看页面" extra={<Button
                ...
                <Descriptions title="简牍信息">
                    <Descriptions.Item label="出土地/单位">{update_data.chutudi}</Descriptions.Item>
                    <Descriptions.Item label="题目">{update_data.timu}</Descriptions.Item>
            ...
            <Card size="lage" title="单条信息新增页面" extra={<Button
                ...
                <Form form={form} name="control-hooks" style={{width: 400}}>
                    ...
                    > <Input onChange={(value) => {
                        add_data.chutudi = value.target.value
                    }}
                    /> </Form.Item>
                    <Form.Item
                        name='timu'
                        label='题目'
                    > <Input onChange={(value) => {
                        add_data.timu = value.target.value
                    }}
                    /> </Form.Item>
                    <Form.Item
                        name='wenben'
